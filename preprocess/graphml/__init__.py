...
class GraphMLCreator:
    ...
    # 여기 이렇게 만든 이유, createGraphMLWithThreshold로 가기!
    # 노드 만들고 엣지 만들기. 이후: 엣지 먼저 만들기 bcs. threshold 넘어가는 애들만 keep하려고.
    # 노드 먼저 하면 , A-B 남기고 C-D버리려고 할 때 엣지 없는 애들이 노드만 남아서 혼자 있음.
    # so, 엣지 구성하는 왼쪽,오른쪽 단어를 filtered word list에 담아 이걸로 노드를 만듦.
    # 그 단어의 카운트값도 같이 넣어주면 노드 크기도 나올 것. 코드 그렇게 생겨 있음!
